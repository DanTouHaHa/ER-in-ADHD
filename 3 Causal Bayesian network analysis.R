...
# Chosing a subset of variables: 'res_BAS','res_Cognition','res_Emotion','res_Rawscore'
...
######如果涉及到多组被试或者交叉项，需要 emmeans包进行事后检验与简单效应分析
library(emmeans)
ABCD_CBCL_Emotion_Cognition_MOtivation_FL2 <- read_excel("ABCD_CBCL_ECM_FL2_PRS_mean_for_Bayes.xlsx")
#section 数据清洗----
###### 认知按照不调整的数据计算
###### 认知、情绪、动机、症状区分维度，先都按照不区分维度进行计算。
ABCD_CBCL_Emotion_Cognition_MOtivation_FL2[3:45] <- lapply(ABCD_CBCL_Emotion_Cognition_MOtivation_FL2[3:45], as.numeric)
...
del7 <- union(del6,del5)
ABCD_CBCL_Emotion_Cognition_MOtivation_FL2 <- ABCD_CBCL_Emotion_Cognition_MOtivation_FL2[-del7,]
del8 <- which(ABCD_CBCL_Emotion_Cognition_MOtivation_FL2$SCORE == "NA")
ABCD_CBCL_Emotion_Cognition_MOtivation_FL2 <- ABCD_CBCL_Emotion_Cognition_MOtivation_FL2[-del8,]
ABCD_CBCL_Emotion_Cognition_MOtivation_FL2[50] <- lapply(ABCD_CBCL_Emotion_Cognition_MOtivation_FL2[50], as.numeric)
ABCD_CBCL_Emotion_Cognition_MOtivation_FL2[51] <- lapply(ABCD_CBCL_Emotion_Cognition_MOtivation_FL2[51], as.numeric)
ABCD_CBCL_Emotion_Cognition_MOtivation_FL2[59:68] <- lapply(ABCD_CBCL_Emotion_Cognition_MOtivation_FL2[59:68], as.numeric)
ABCD_CBCL_Emotion_Cognition_MOtivation_FL2[73:75] <- lapply(ABCD_CBCL_Emotion_Cognition_MOtivation_FL2[73:75], as.numeric)
ABCD_CBCL_Emotion_Cognition_MOtivation_FL2[77] <- lapply(ABCD_CBCL_Emotion_Cognition_MOtivation_FL2[77], as.numeric)
...
BMI <- read_excel("BMI匹配完被试.xlsx")
ABCD_CBCL_Emotion_Cognition_MOtivation_FL2 <- left_join(ABCD_CBCL_Emotion_Cognition_MOtivation_FL2, BMI,  by="src_subject_id")
ABCD_CBCL_Emotion_Cognition_MOtivation_FL2[89] <- lapply(ABCD_CBCL_Emotion_Cognition_MOtivation_FL2[89], as.numeric)
Puberty <- read_excel("Puberty.xlsx")
ABCD_CBCL_Emotion_Cognition_MOtivation_FL2 <- left_join(ABCD_CBCL_Emotion_Cognition_MOtivation_FL2, Puberty,  by="src_subject_id")
ABCD_CBCL_Emotion_Cognition_MOtivation_FL2[90:92] <- lapply(ABCD_CBCL_Emotion_Cognition_MOtivation_FL2[90:92], as.numeric)

#section 先控制协变量，得到残差进行Bayes----
###### 需要先控制掉协变量，将需要进行推断的变量先回归协变量
library(bnlearn)
library(gtools)
###选取需要的多列
Data <- select(ABCD_CBCL_Emotion_Cognition_MOtivation_FL2, BISBAS,BAS,TotalCognition, TotalEmotionScore,
               rawscore, rawscore_FL2, Age2, sex2, demo_comb_income_v2, inattention, impul_hyper,SCORE, BMI, A_puberty,
               demo_prnt_ed_v2, race_ethnicity,  site_id_l, rel_family_id)
...
model_Cognition <- lm(TotalCognition ~ Age2 + sex2 + demo_comb_income_v2 +   
                        demo_prnt_ed_v2 +as.factor(race_ethnicity) +as.factor(site_id_l),
                      data = Data)
summary(model_Cognition)
res_Cognition = model_Cognition$residuals
...
model_score <- lm(SCORE ~ Age2 + sex2 + demo_comb_income_v2 +  
                    demo_prnt_ed_v2 + as.factor(race_ethnicity) +as.factor(site_id_l), 
                  data = Data)
summary(model_score)
res_score = model_score$residuals

#####将5个values合并成dataframe,,,,可以换成res_inattention, res_impul
Data_Bayes_BAS <- cbind(res_BAS,res_Cognition,res_Emotion,res_Rawscore, res_score)
Data_Bayes_BAS <- as.data.frame(Data_Bayes_BAS)
variables_BAS <- c('res_BAS','res_Cognition','res_Emotion','res_Rawscore','res_score')

######################### DEALING WITH CONTINUOUS VARIABLES ###############################
# Chosing a subset of variables: 'res_BAS','res_score','res_Cognition','res_Emotion','res_Rawscore'
final_variables_BAS <- c(1,2,3,4,5)
Data_Bayes_BAS = Data_Bayes_BAS[,final_variables_BAS]
...
for(algo in alogorithms){
  for(score in scores){
    ...
    print(score)
    models <- boot.strength(data = Data_Bayes_BAS,R=5000,algorithm = algo,algorithm.args = list(score=score))
    best_models <- averaged.network(models,threshold = 0.70)
    fitted_model <- bn.fit(best_models,data = Data_Bayes_BAS)
    Learned_Bayesian_network <- graphviz.plot(fitted_model,highlight = list('fill'),layout = 'fdp',render = TRUE)
  }
}
